...
class UPerNetDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        # FIX: Originally initialized a ModuleList of 4 layers but only used the last one.
        # We now only initialize the single layer we use to avoid DDP unused parameter errors.
    ...
    def forward(self, feature_pyramid, input_shape):
        ...
        # FIX: Use the single lateral_conv directly
        lateral_output = self.lateral_conv(features)
...
class PrithviCloudImputer(nn.Module):
    ...
    def _load_pretrained_weights(self, weights_path, num_frames):
        ...
        # --- OPTIMIZED LOADING (Rank 0 reads, others receive) ---
        if dist.is_initialized():
            rank = dist.get_rank()
            if rank == 0:
                print(f"Loading Prithvi weights from: {weights_path} (Rank 0 Only)")
                try:
                    # Load to CPU to avoid GPU OOM during broadcast prep
                    checkpoint = torch.load(weights_path, map_location="cpu", weights_only=True)
            ...
            # Broadcast the loaded dictionary to all other GPUs
            # This uses the High-Speed Network (HSN) instead of the Slow File System
            obj_list = [checkpoint]
            dist.broadcast_object_list(obj_list, src=0)
            checkpoint = obj_list[0]
            
            if checkpoint is None:
                raise RuntimeError("Checkpoint failed to load or broadcast!")
        else:
            # Fallback for single GPU/No-DDP
            print(f"Loading Prithvi weights from: {weights_path}")
            checkpoint = torch.load(weights_path, map_location="cpu", weights_only=True)
        # --------------------------------------------------------
